...
# MONKEY-PATCH buggy Treex::PML::Node subroutine
import PML;
import TredMacro;
...
# Work-around for buggy Treex::PML::Schema::Container
# Container.pm has two references to non-existent Treex::PML::FSFactory.
# Because the subroutine is too long to patch, we make FSFactory available
# as a (trivial) subclass of Treex::PML::Factory
{
...
sub create_new_son
{
    # don't add children to leaf nodes (i.e. with a "root" attribute)
...
sub create_new_parent
{
    ...
    $this->{rel} = 'hd';
    if (defined($this->{pos}) && $this->{pos} eq 'name')
      { $node->{cat} = 'np' }
    if (defined($this->{pos}) && $this->{pos} eq 'noun')
      { $node->{cat} = 'np' }
    if (defined($this->{pos}) && $this->{pos} eq 'adj')
      { $node->{cat} = 'ap' }
    if (defined($this->{pos}) && $this->{pos} eq 'adv')
      { $node->{cat} = 'advp' }
    if (defined($this->{pos}) && $this->{pos} eq 'det')
      { $node->{cat} = 'detp' }
...
sub EditPos{
    if (!$this->children) {
        ChangingFile(EditAttribute($this,'pos'));
    } else {
        ChangingFile(0);
    }
}
...
#bind EditRoot to l menu Edit Root
sub EditRoot{
    if (!$this->children) {
        ChangingFile(EditAttribute($this,'root'));
    } else {
        ChangingFile(0);
    }
}
...
rootstyle: #{balance:spread}#{Line-coords:n,n,n,p,p,p}#{EdgeLabel-coords:n,n-25}#{NodeLabel-halign:center}#{NodeLabel-yadj:-7}#{Node-textalign:center}
node: #{brown}${cat}${pos} #{grey40}${postag}
node: #{blue}${word} <? 
    "(#{brown}".$${root}.'#{grey40} : '.$${lemma}.")" if $${lemma}; ?>
...
# $PYTHON2PATH = 'C:\programs2\python27\python.exe'; # TEMPORARY!!!! 
...
sub AssignCatOfParent {
    ...
    if ($pos eq "noun" || $pos eq "pron" || $pos eq "name")
      { $this -> parent -> {cat} = "np" }
    if ($pos eq "prep")
      { $this -> parent -> {cat} = "pp" }
    if ($pos eq "adj")
