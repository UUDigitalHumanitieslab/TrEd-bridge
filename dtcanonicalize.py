...
class TreeNode:
    ...
    def write_xml(self, outfile=sys.stdout, indent=''):
...
class DTFile:
    ...
    def write_xml(self, outfile, encoding='UTF-8'):
        ...
        # de root node
        outfile.write('<alpino_ds version="1.3">\n')
        ...
        # metadata
        if self.metadata:
            self.metadata.writexml(outfile, indent='  ',
                                   addindent='  ')
            print('', file=outfile)
...
class DTParser:
    ...
    def do_metadata(self, xmlnode, container):
        container.set_metadata(xmlnode)
...
class Canonicalizer:
    ...
    def __init__(self, dtparser, filename):
        self.dtfile = dtparser.parsefile(filename)
    ...
    def fill_index_tables(self, node=None):
        ...
        if node.gettype() == node.INDEXED:
            ...
            if index in self.indexed_nodes:
                raise self.Error(
                    self.filename, 'index "%s" is used more than once' % index.encode('utf-8'))
    ...
    def check_indices(self):
        ...
        for index in self.indexed_nodes.keys():
            if index not in self.index_nodes:
                ...
                raise self.Error(
                    self.filename, 'no target node for index "%s"' % index.encode('utf-8'))
        ...
        for index in self.index_nodes.keys():
            if index not in self.indexed_nodes:
                # een kale verwijzing
                raise self.Error(
                    self.filename, 'no source node for index "%s"' % index.encode('utf-8'))
    ...
    def calculate_range(self, node, indices_in_progress):
        ...
        for child in node.children:
            childranges.append(self.calculate_range(
                child, indices_in_progress))
        ...
        node.end = end
        node.dominated_words = wordlist
        # fixme? returning wordlist should be enough...
        return begin, end, wordlist
    ...
    def get_string_positions(self, node):
        ...
        try:
            ...
            raise self.Error(
                self.filename, 'non-numeric begin position "%s"' % node['begin'].encode('utf-8'))

        try:
            end = int(node['end'])
        except ValueError:
            raise self.Error(
                self.filename, 'non-numeric end position "%s"' % node['end'].encode('utf-8'))
    ...
    def check_leaf_node(self, node):
        ...
        for attr in 'pos', 'begin', 'end', 'root', 'word':
            if not node.has_key(attr):
                raise self.Error(
                    self.filename, "leaf node without @%s : %s!" % (attr, node.attributes))
    ...
    def reorder_tree(self, node, id=0):
        ...
        # sort the children
        node.children.sort(key=self.node_key)
    ...
    def node_key(node):
        """
        Construct the key for sorting nodes:
           - try sorting (comparing) on end position,
           - fall back on wordlist comparison
           - then fall back on rel values

        This is identical to the old node_cmp() sort,
        except that there is no way to raise an error
        if two nodes compare identical.
        """
        return (node.end, node.dominated_words, node['rel'])
...
if __name__ == '__main__':
    ...
    parser = OptionParser(
        "usage: %prog [options] <alpino xml-file(s)>", prog=progname)
    ...
    if options.stdin:
        ...
        for file in args:
            try:
                ...
                tempfileobj = open(tempfd, "w", encoding='utf-8')
                ...
                if os.name == 'posix':
                    ...
                    # We'll do our best to avoid being interrupted:
                    with open(tempname, encoding="utf-8") as tmp:
                        tmptext = tmp.read()
                ...
                print('%s: while parsing %s: %s' %
                      (progname, file, err), file=sys.stderr)
                error_occurred = True
